<template>
  <div>
        <el-select v-model="provinceCode" filterable placeholder="请选择省份">
            <el-option
...
export default {
  ...
  watch: {
    ...
    provinceCode: function(newProvinceCode, oldProvinceCode){
        ...
        $this.cascadeCities = []
        this.allCities.forEach((item, index) => {
          if(item.code.slice(0,2) === newProvinceCode.slice(0,2))
            $this.cascadeCities.push(item)
        })
    },
    city: function(newCity, oldCity){
      this.$emit('change', this.province, newCity)
    }
  },
  methods: {
