...
<script>
  ...
  export default {
    ...
    created(){
        ...
        banner.getOne(id).then(res => {
            $this.bannerForm = res.data
            if($this.bannerForm.redirectType === 'NONE'){
                $this.redirectTypeRadioValue = 3
            }else if($this.bannerForm.redirectType === 'IMAGEANDTEXT'){
                $this.redirectTypeRadioValue = 2
            }else if($this.bannerForm.redirectType === 'LINK'){
      ...
      submitForm(formName, publish) {
        this.$refs[formName].validate((valid) => {
          if (valid) {
            ...
            banner.updateOne(this.bannerForm).then(res => {
